...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="https://cdn.jsdelivr.net/npm/tail.select@0.5.15/css/bootstrap4/tail.select-default.min.css">
 ...
 $(document).ready(function() {
      ...
      swal({
        ...
        if (ok) {
          $.ajax({
            ...
            type: 'POST',
            data:{_token:token,input_id:input_id,status:status},
            success:function(data) /*dữ liệu(data) trả về bên function*/
            {   
              swal({
   ...
   $(document).on('change','#nhacungcap',function(){
     ...
     if (vendor_id) {
       $.ajax({
        ...
        type: 'POST',
        dataType:'json',
        data:{_token:token,vendor_id:vendor_id},
        success:function(data) /*dữ liệu(data) trả về bên function*/
        {   
          var html = '';
          html +=`<tr>
          ...
          {
            html += `<option value="${item}" id="product_${item}">${data[item]}</option>`;
          }
          html += `</select>
          </td>
          <td>
  ...
  if (vendor_id) {
   $.ajax({
    ...
    type: 'POST',
    dataType:'json',
    data:{_token:token,vendor_id:vendor_id,id:id},
    success:function(data) /*dữ liệu(data) trả về bên function*/
    {   
     swal({
